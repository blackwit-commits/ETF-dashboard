...
function openAllocationModal(sym) { const ctx = _computeAllocContext(sym); _allocCtx = ctx; if(ctx.total <= 0) { alert("설정 탭에서 초기 시드머니를 먼저 입력해주세요."); return; } const m = document.getElementById('allocationModal'); m.classList.remove('hidden'); m.classList.add('flex'); document.getElementById('allocTotalAsset').innerText = '$' + ctx.total.toLocaleString(undefined,{maximumFractionDigits:2}); const allocKrwEl = document.getElementById('allocTotalAssetKrw'); if (allocKrwEl) allocKrwEl.innerText = formatKrw(ctx.total); const fxEl = document.getElementById('allocFxRate'); if (fxEl) { var _r = (typeof _liveUsdKrw !== 'undefined' && _liveUsdKrw) ? _liveUsdKrw : ((globalData && globalData.rate) || 0); fxEl.innerText = '₩' + Math.round(_r).toLocaleString() + ' / $1'; } const oEl = document.getElementById('allocOther'); if (oEl) oEl.innerText = ctx.otherPct + '% · $' + Math.round(ctx.otherUsd).toLocaleString(); const rEl = document.getElementById('allocRemain'); if (rEl) rEl.innerText = ctx.remainPct + '% · $' + Math.round(ctx.remainUsd).toLocaleString(); const existing = (sym && portfolios[sym] && portfolios[sym].config && (portfolios[sym].config.alloc != null)) ? portfolios[sym].config.alloc : null; const pct = existing != null ? existing : Math.min(10, Math.floor(ctx.remainPct)); document.getElementById('allocPercent').value = pct; calcAllocFromPct(); }
