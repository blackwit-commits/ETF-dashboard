...
function calculatePlan() {
    ...
    if (boosterOn && boosterStages > 0 && drops.length > 0) {
        ...
        <tr class="border-b border-slate-800/50 hover:bg-slate-800/30 transition bg-slate-800/20">
            <td class="p-2 text-center font-medium leading-tight"><div class="text-slate-300">${stageNum}차</div><span class="inline-block text-[8px] font-black px-1 py-0.5 rounded bg-red-500/20 text-red-300 mt-0.5 whitespace-nowrap"><i class="fa-solid fa-rocket text-[7px] mr-0.5"></i>부스터</span><div class="text-[8px] text-red-400/70 mt-0.5">${Math.round(bDrop)}%</div></td>
            <td class="p-2 text-center text-blue-300 font-bold align-middle">$${targetPrice.toFixed(2)}</td>
            <td class="p-2 text-center align-middle">${actualPriceCell(actualBuyPrice, targetPrice)}</td>
            <td class="p-2 text-center align-middle">${actualQtyCell(qty, boughtQty)}</td>
            <td class="p-2 text-center align-middle">${statusBadge}</td>
...
function openAllocationModal(sym) { const ctx = _computeAllocContext(sym); _allocCtx = ctx; if(ctx.total <= 0) { alert("설정 탭에서 초기 시드머니를 먼저 입력해주세요."); return; } const m = document.getElementById('allocationModal'); m.classList.remove('hidden'); m.classList.add('flex'); document.getElementById('allocTotalAsset').innerText = '$' + ctx.total.toLocaleString(undefined,{maximumFractionDigits:2}); const allocKrwEl = document.getElementById('allocTotalAssetKrw'); if (allocKrwEl) allocKrwEl.innerText = formatKrw(ctx.total); const fxEl = document.getElementById('allocFxRate'); if (fxEl) { var _r = (typeof _liveUsdKrw !== 'undefined' && _liveUsdKrw) ? _liveUsdKrw : ((globalData && globalData.rate) || 0); fxEl.innerText = '₩' + Math.round(_r).toLocaleString() + ' / $1'; } const oEl = document.getElementById('allocOther'); if (oEl) oEl.innerText = ctx.otherPct + '% · $' + Math.round(ctx.otherUsd).toLocaleString(); const oKrw = document.getElementById('allocOtherKrw'); if (oKrw) oKrw.innerText = formatKrw(ctx.otherUsd); const existing = (sym && portfolios[sym] && portfolios[sym].config && (portfolios[sym].config.alloc != null)) ? portfolios[sym].config.alloc : null; const pct = existing != null ? existing : Math.min(10, Math.floor(ctx.remainPct)); document.getElementById('allocPercent').value = pct; calcAllocFromPct(); }
function calcAllocFromPct() { const pct = parseFloat(document.getElementById('allocPercent').value)||0; const total = (_allocCtx && _allocCtx.total) || getTotalEquityUSD(); const amt = total * (pct/100); document.getElementById('allocAmount').value = amt.toFixed(2); updateKrwHint(amt); updateAllocWarn(pct); updateAllocRemainLive(pct); }
function calcAllocFromAmt() { const amt = parseFloat(document.getElementById('allocAmount').value)||0; const total = (_allocCtx && _allocCtx.total) || getTotalEquityUSD(); const pct = total>0 ? (amt / total) * 100 : 0; document.getElementById('allocPercent').value = pct.toFixed(1); updateKrwHint(amt); updateAllocWarn(pct); updateAllocRemainLive(pct); }
// 남은 여력 = 총자산 − 다른 종목 할당 − 현재 입력분 (실시간)
function updateAllocRemainLive(pct) {
    if (!_allocCtx) return;
    var leftPct = Math.round((_allocCtx.remainPct - (pct || 0)) * 10) / 10;
    var leftUsd = _allocCtx.total * leftPct / 100;
    var neg = leftPct < -0.05;
    var cls = neg ? 'text-red-400' : 'text-emerald-400';
    var rEl = document.getElementById('allocRemain');
    if (rEl) { rEl.className = cls + ' font-black'; rEl.innerText = leftPct.toFixed(1) + '% · $' + Math.round(leftUsd).toLocaleString(); }
    var kEl = document.getElementById('allocRemainKrw');
    if (kEl) { kEl.className = 'text-[10px] font-bold ' + (neg ? 'text-red-400/80' : 'text-emerald-400/70'); kEl.innerText = formatKrw(leftUsd); }
}
